...
3. relay スクリプトをダウンロード：
   ```bash
   curl -o ~/relay_termux.py https://raw.githubusercontent.com/nkmr-lab/DualDraw/main/relay_termux.py
   ```
...
6. 「ペアリングコードによるデバイスのペアリング」をタップすると、**ペアリング用ポート**と6桁コードが表示される。Termux で：
   ```bash
   adb pair 127.0.0.1:<ペアリング用ポート>
   ```
   6桁コードを入力

> **注意：** ペアリング用ポートと接続用ポートは別の番号です。ペアリングダイアログに表示されるポートはペアリング専用で、ワイヤレスデバッグのメイン画面に表示されるポートが接続用です。

### 毎回の起動手順（Termux）

1. ワイヤレスデバッグ設定画面で**接続用ポート**を確認（再起動のたびに変わる）

2. Termux で接続＆起動：
   ```bash
   adb connect 127.0.0.1:<接続用ポート>
   python ~/relay_termux.py
   ```

3. DualDraw アプリを起動 → `RAW touch=✓ stylus=✓` と表示されれば成功
